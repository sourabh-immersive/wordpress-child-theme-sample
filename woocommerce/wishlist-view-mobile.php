...
<table
	...
	<tr>
		...
		<?php if ( $show_stock_status ) : ?>
			<?php $column_count ++; ?>
			<th class="product-stock-status" data-title="Status">
				<span class="nobr">
		...
		foreach ( $wishlist_items as $item ) :
			...
			if ( $product && $product->exists() ) :
				...
				<tr id="yith-wcwl-row-<?php echo esc_attr( $item->get_product_id() ); ?>" data-row-id="<?php echo esc_attr( $item->get_product_id() ); ?>">
					...
					<?php if($product_type != 'grouped'): if ( $show_price || $show_price_variations ) : ?>
					...
					<?php if ( $show_stock_status ) : ?>
						<td class="product-stock-status" data-title="Status">
							...
							if($product->backorders_allowed()) { 
								echo 'Available on Pre-order';
							}
							else{
								echo 'out-of-stock' === $stock_status ? '<span class="wishlist-out-of-stock">' . esc_html( apply_filters( 'yith_wcwl_out_of_stock_label', __( 'Out of stock', 'yith-woocommerce-wishlist' ) ) ) . '</span>' : '<span class="wishlist-in-stock">' . esc_html( apply_filters( 'yith_wcwl_in_stock_label', __( 'In Stock', 'yith-woocommerce-wishlist' ) ) ) . '</span>';
							}
							 ?>
					...
					<?php if ( $show_last_column ) : ?>
						<td class="product-add-to-cart">
							...
							<?php endif; ?>
							<?php if($product_type == 'grouped'):?>
							<a href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item->get_product_id() ) ) ); ?>">Shop the range</a>
							<?php endif; ?>
						</td>
					<?php endif; ?>
